...
def load_annotation_data(annotation_file, taxa_of_interest):
    ...
    types = data['type'].values.astype(np.int64)

    return locs, taxa, types
...
def get_annotation_data(params):
    ...
    locs, _, types = load_annotation_data(annotation_file, taxa_of_interest) # has only 2 labels, drop labels
    ...
    ds = datasets.BinaryLocationDataset(locs, labels, classes, types, class_to_taxa, params['input_enc'], params['device'])
